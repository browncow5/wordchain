...
if __name__ == "__main__":
    logger.info("Running main funct")

    if len(sys.argv) < 3:
        logger.info("Usage: python script.py <root_word> <length>")
        sys.exit(1)

    root_word = sys.argv[1]
    length = int(sys.argv[2])

    logger.info(f"Starting recursive compound search for root word '{root_word}' with length {length}...")
    result = [root_word] + recursive_compound_search(root_word, length)
    logger.info(f"Result: {result}")
"""

""" Replaced by get_word_frequency
@tracer.wrap(service="compound-words", resource="is-word")
def is_word(inputWord): 
#    logger.info(f"Checking if '{inputWord}' is a standalone word with a frequency greater than 1.")
    url = f"https://api.datamuse.com/words?sp={inputWord}&max=1&md=f"
    response = requests.get(url)
    data = response.json()
    ...
    if len(data) > 0:
        ...
        if data[0]['word'] == inputWord:
            ...
            for tag in tags:
                if tag.startswith("f:"):
                    frequency = float(tag[2:])
                    is_standalone = frequency > 1
                    logger.info(f"'{inputWord}' is a standalone word with a frequency({frequency}) greater than 1: {is_standalone}")
